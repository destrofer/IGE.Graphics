...
namespace IGE.GUI {
	public class ThinBorder : ISizedRenderable2D {
		...
		public ThinBorder(Color4 color) {
			m_Color = color;
		}
		
		public ThinBorder(DomNode node) {
			Load(node);
		}
		...
		public virtual void Render(int x, int y, int width, int height) {
			...
			GL.Begin(BeginMode.LineLoop);
				GL.Vertex2(x + 0.5f, y + height - 0.5f);
				GL.Vertex2(x + width - 0.5f, y + height - 0.5f);
				GL.Vertex2(x + width - 0.5f, y + 0.5f);
				GL.Vertex2(x + 0.5f, y + 0.5f);
			GL.End();
			GL.PopAttrib();
		}
